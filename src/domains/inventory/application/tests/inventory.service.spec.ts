...
describe('Inventory — InventoryService', () => {
  ...
  it('should receive stock and recompute available (FR-INV-010)', async () => {
    lockedRow = { variantId: 'v1', onHand: 2, reserved: 0, available: 2, lowStockThreshold: 3 };
    const result = await service.receive('v1', 50, 'supplier_delivery');
  ...
  it('should reject receive with quantity <= 0 (FR-INV-010)', async () => {
    await expect(service.receive('v1', 0, 'x')).rejects.toBeInstanceOf(BadRequestException);
  });

  it('should reject receive with a missing reason', async () => {
    await expect(service.receive('v1', 5, '  ')).rejects.toBeInstanceOf(BadRequestException);
  });

  it('should reject adjust with quantity_delta == 0 (FR-INV-011)', async () => {
    await expect(service.adjust('v1', 0, 'x')).rejects.toBeInstanceOf(BadRequestException);
  });

  it('should apply a negative adjust that stays >= 0 (FR-INV-012)', async () => {
    lockedRow = { variantId: 'v1', onHand: 5, reserved: 0, available: 5, lowStockThreshold: 3 };
    const result = await service.adjust('v1', -2, 'damaged');
  ...
  it('should reject an adjust that would make on_hand negative → NEGATIVE_ON_HAND (BR-INV-2)', async () => {
    lockedRow = { variantId: 'v1', onHand: 2, reserved: 0, available: 2, lowStockThreshold: 3 };
    await expect(service.adjust('v1', -5, 'damaged')).rejects.toBeInstanceOf(ConflictException);
    expect(savedRow).toBeNull(); // row unchanged
  });

  it('should 404 when receiving against a non-existent record', async () => {
    lockedRow = null;
    await expect(service.receive('missing', 5, 'x')).rejects.toBeInstanceOf(NotFoundException);
  });
